...
- **`[userenv.shared]` (and other `[userenv.*]`) in `.replit`** — stored as
...
- **Encrypted Secrets store** — values are hidden; `viewEnvVars()` shows them
...
it "looks" like a managed env var. Auditing only `.env` misses this — check
`.replit` too.
...
support) — the agent cannot run git / rewrite history. After rotation the four
...
`SUPABASE_URL`: PostgREST read (publishable/anon → 200), auth `/auth/v1/settings`
+ `/auth/v1/admin/users` (service_role → 200). A `42501` table-grant 403 means the
key is valid but the role lacks a GRANT — not a credential failure (an invalid
key returns 401).
