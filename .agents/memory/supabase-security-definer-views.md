...
# security_invoker=false views bypass RLS — the view WHERE clause is the only gate
...
that the base-table RLS no longer protects anything reached through the view — a
...
- A `CREATE OR REPLACE VIEW` cannot rename/reorder/retype existing columns — keep the
...
queries, so a child policy that calls it does **not** re-trigger the child's own RLS →
