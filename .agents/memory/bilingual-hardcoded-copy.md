...
description: This AR/EN app has screens/components that hardcode single-language copy instead of using t()/lang checks — bilingual or polish sweeps must grep for it.
---

# Bilingual hardcoded-copy trap

This storefront is bilingual (AR-RTL / EN-LTR), but not every screen routes its
copy through `t()` or `lang === "ar" ? … : …`. Some files hardcode Arabic-only
...
review missed that the contact surface rendered Arabic in EN mode — a real
...
using `useLanguage()` — note that editing `src/lib/locales/*` is often
...
meta `title`/`description` — keep them consistent with sibling routes (the rest
of the app uses English titles like `"Shop — GARHY | HYPER"`).
